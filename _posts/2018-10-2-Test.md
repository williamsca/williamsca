...
The economist Harold Hotelling pointed out that two store owners will naturally locate their stores [right next](https://en.wikipedia.org/wiki/Hotelling%27s_law) to each other so that they will appeal to the largest possible number of buyers. 
...
All the math in this first section can safely be ignored. The story is quite simple and can be comprehended from the pictures.
